...
const isFirebaseDisabled = !auth || auth.currentUser === undefined;
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    // Skip Firebase auth state listener if Firebase is disabled (dev mode with placeholder config)
    if (isFirebaseDisabled || isDevMode) {
      console.log('⚠️  Firebase auth state listener disabled in dev mode - using local auth only');
      return;
    }
  ...
  const signup = async (name, email, password, role = 'user') => {
    // In dev mode, use local backend registration instead of Firebase
    if (isDevMode || isFirebaseDisabled) {
      try {
  ...
  const signInWithGoogle = async () => {
    // In dev mode, Google sign-in is not available
    if (isDevMode || isFirebaseDisabled) {
      toast({
        title: "Google Sign-In Unavailable",
        description: "Google sign-in is disabled in development mode. Please use email/password login.",
        variant: "destructive"
      });
      return null;
    }
  ...
  const logout = async () => {
    try {
      ...
      if (provider === 'firebase' && !isFirebaseDisabled) {
        await signOut(auth);
      }
  ...
  const setupRecaptcha = (containerId) => {
    if (isDevMode || isFirebaseDisabled) {
      console.warn('⚠️  Recaptcha disabled in dev mode');
      return;
    }
  ...
  const sendOtp = async (phoneNumber) => {
    if (isDevMode || isFirebaseDisabled) {
      toast({
        title: "OTP Login Unavailable",
        description: "Phone authentication is disabled in development mode. Please use email/password login.",
        variant: "destructive"
      });
      return null;
    }
