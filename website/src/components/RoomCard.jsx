...
const RoomCard = ({ room, onClick, index, viewMode = 'grid' }) => {
  ...
  const getPriceDisplay = () => {
    ...
    // PGs are monthly, everything else is per night
    const isPG = categoryLower.includes('pg') || categoryLower.includes('paying guest');
    const pricingUnit = isPG ? '/month' : '/night';
      ...
      >
        <div className="relative w-80 h-48 overflow-hidden flex-shrink-0">
          ...
          <div className="absolute top-3 left-3">
            <PropertySourceBadge property={room} size="sm" />
          </div>
        </div>
