...
Write-Host "Testing check-in for booking: $bookingId"

try {
    $response = Invoke-RestMethod -Uri "$baseUrl/internal/bookings/$bookingId/checkin" -Method POST -Body $checkinData -Headers $headers
    Write-Host "✅ Check-in successful"
    Write-Host ($response | ConvertTo-Json -Depth 3)
} catch {
    Write-Host "❌ Check-in failed: $($_.Exception.Message)"
